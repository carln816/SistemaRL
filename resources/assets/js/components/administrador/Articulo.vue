                            ...
                            <tbody>
                                <tr v-for="articulo in arrayArticulo" :key="articulo.id">
                                    <td>
                                        ...
                                        <template>
                                        <button type="button" class="btn btn-danger btn-sm" data-tooltip title="Eliminar" @click="eliminarReg(articulo)" v-if="idrol==1">
                                            <i class="fa fa-minus-circle" aria-hidden="true"></i>
                                        </button>
                                        </template>
                                        <template v-if="articulo.condicion"> <!--si la condicion es 1 lo que hago es moestro un boton para desactivar el metodo -->
                                            <button type="button" class="btn btn-light" @click="desactivarArticulo(articulo.id)" v-if="idrol==1">
                                                <i class="fa fa-window-close" aria-hidden="true"></i>
                                            </button>
                                        </template>
                                        <template v-else><!--si la categoria no esta activa, mostrate un boton para llamar al metodo activar-->
                                            <button type="
                                            button" class="btn btn-light" @click="activarArticulo(articulo.id)" v-if="idrol==1">
                                    ...
                                    <td>
                                        <div v-if="articulo.condicion"><!--  si la condicion es igual 1-->
                        ...
                        <div class="modal-body">
                            <form action="" method="post" enctype="multipart/form-data" class="form-horizontal">
                                ...
                                <div class="form-group row">
                                    ...
                                    <p class="text-red-600" v-if="errors.nombre">{{errors.nombre}}</p>
                                    <div class="col-md-9">
                                        <input type="text" v-model="nombre" class="form-control" placeholder="Nombre de artículo">                                        
                                    </div>
                                </div>
                                <div class="form-group row" v-if="idrol==1">
                                    <label class="col-md-3 form-control-label" for="text-number">Precio Venta</label>
                                    <div class="col-md-9">
                                        <input type="number" v-model="precio_venta" class="form-control" placeholder="">                                        
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label class="col-md-3 form-control-label" for="text-number">Stock</label>
                                    <div class="col-md-9">
    ...
    export default {
        data (){
            return {
                ...
                tipoAccion : 0,
                errorArticulo : 0,
                errorMostrarMsjArticulo : [],//arrays donde almaceno los posibles errores que tenga
                pagination : {
                    'total' : 0,
                    'current_page' : 0,
                    'per_page' : 0,
        ...
        methods : {
            listarArticulo (page,buscar,criterio){
                ...
                axios.get(url).then(function (response) { //mediante el metodo de axios obtengo todos los registros de la tabla categorias
                    ...
                    me.arrayArticulo = respuesta.articulos.data;// almacene todos el contenido del objeto response
            ...
            cambiarPagina(page,buscar,criterio){
               let me = this;
                ...
                me.listarArticulo(page,buscar,criterio);// le envia valores al metodo
            },
            ...
            registrarArticulo(){
               ...
               axios.post('/articulo/registrar',{
                ...
                // si se ejecuta de manera correcta llamamos al metodo cerrarmodal y listarcategoria para actualizar el listado
                   swal(
                        'Registrado!',
                        'El articulo ha sido registrado!',
                        'success'
                    )
                    me.cerrarModal();
                    me.listarArticulo(1,'','nombre');//le enviamos qe muestre nuestra primera pagina el fltro  
            ...
            validarArticulo(){
                ...
                //si tengo un error evaluamos nuestro array y nuestra variable error Articulo va a pasar 1 cuando tenga al menos 1 error.
            ...
            abrirModal(modelo, accion, data = []){ //estructura multiple donde evalua el parametro
                switch(modelo){// en caso el texto sea igual al modelo categoria se agrega otra seleccion selectiva
                    case "articulo":   // case que validad que nuestra variable sea igual a categoria 
                    {
